...
parse_result() {
  local result="$1" n="$2"
  local rps p50 p99 s200 s201 s500 s503 mem
  rps=$(echo "$result" | grep "Requests/sec" | awk '{print $2}')
  p50=$(echo "$result" | grep "50%" | awk '{print $3}')
  p99=$(echo "$result" | grep "99%" | awk '{print $3}')
  s200=$(echo "$result" | grep "\[200\]" | awk '{print $2}')
  s201=$(echo "$result" | grep "\[201\]" | awk '{print $2}')
  s500=$(echo "$result" | grep "\[500\]" | awk '{print $2}')
  s503=$(echo "$result" | grep "\[503\]" | awk '{print $2}')
...
for C in 100 500 1000 2000; do
  ...
  NODE_OUT=$(mktemp)
  PY_OUT=$(mktemp)
  $HEY -n "$N_HALF" -c "$HALF" -m POST -H "X-Orva-API-Key: $API_KEY" \
    -H "Content-Type: application/json" -d '{"name":"mixed"}' -t 120 "$NODE_URL" >"$NODE_OUT" 2>&1 &
  PID_NODE=$!
  $HEY -n "$N_HALF" -c "$HALF" -m POST -H "X-Orva-API-Key: $API_KEY" \
    -H "Content-Type: application/json" -d '{"dept":"eng"}' -t 120 "$PY_URL" >"$PY_OUT" 2>&1 &
  PID_PY=$!
  wait $PID_NODE $PID_PY 2>/dev/null
  RESULT_NODE=$(<"$NODE_OUT")
  RESULT_PY=$(<"$PY_OUT")
  rm -f "$NODE_OUT" "$PY_OUT"

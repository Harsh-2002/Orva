...
- `performance/sqlite_index_ab.py` — scratch-only backup-copy A/B for execution-index hypotheses. It opens the source DB read-only, drops requested indexes only in a temporary candidate copy, alternates insert batches and reports read plans/timings; its Python-driver results do not prove real Orva capacity. The CI-discovered `e2e/unit/test_sqlite_index_ab.py` covers its refusal and source/copy safety contract.

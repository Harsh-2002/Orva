...
- `performance/sqlite_index_ab.py` — scratch-only backup-copy A/B for execution-index or writer-cache hypotheses. It opens the source DB read-only, changes only a temporary candidate copy, and reports read plans. With `--driver-test-binary`, the opt-in `database.TestExecutionWriterSnapshotProbe` uses Orva's real writer/driver and verifies row counts plus disk-I/O deltas. `--evict-copy-cache` requests file-scoped cache eviction for both disposable copies, but is advisory. Always run `--identical-control` and reverse copy order: an unchanged control flipped by over 10× from guest page-cache residency. No short copy-based result proves real HTTP capacity. CI-discovered unit tests cover refusal and source/copy safety.

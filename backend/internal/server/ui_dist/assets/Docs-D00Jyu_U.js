...
|---|---|---|---|
`+ne.map(x=>`| ${x.name} | \`${x.id}\` | \`${x.entry}\` | \`${x.deps}\` |`).join(`
`),T=`| Field | Purpose | Behaviour |
|---|---|---|
`+pe.map(x=>`| \`${x.field}\` | ${x.purpose} | ${x.body} |`).join(`
`),d=`| Trigger | Meaning |
|---|---|
`+Ye.map(x=>`| \`${x.name}\` | ${x.desc} |`).join(`
`),N=`| Event | When it fires |
|---|---|
`+$e.map(x=>`| \`${x.name}\` | ${x.when} |`).join(`
`),ce=`| Code | When you see it |
|---|---|
`+fe.map(x=>`| \`${x.code}\` | ${x.when} |`).join(`
`),gt=`| Command | Subcommands | Purpose |
|---|---|---|
`+Ze.map(x=>`| \`orva ${x.cmd}\` | ${x.subs} | ${x.purpose} |`).join(`
...
\`\`\`
`}),me=Ae(!1);let $=null;const ve=async()=>{await Et(je.value)&&(me.value=!0,clearTimeout($),$=setTimeout(()=>{me.value=!1},1500))},te=Ae(!1);let Te=null;const He=async()=>{await Et(je.value);const h=["I'll paste the full Orva documentation right after this message — please use it as your source of truth when I ask questions.","","Reply once you have read it."].join(`

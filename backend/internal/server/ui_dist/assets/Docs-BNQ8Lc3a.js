...
print(r.json())`}]),sn=[{id:`python`,name:`Python 3.14`,entry:`handler.py`,deps:`requirements.txt`,icon:$t},{id:`node`,name:`Node.js 24`,entry:`handler.js`,deps:`package.json`,icon:en}],cn=[{field:`env_vars`,purpose:`Plain config`,body:`Plaintext config stored on the function record. Use for feature flags and non-secret settings.`,icon:ne,iconClass:`text-primary`},{field:`/secrets`,purpose:`Encrypted`,body:`AES-256-GCM at rest. Values decrypt only into the worker environment at spawn time.`,icon:w,iconClass:`text-primary`},{field:`network_mode`,purpose:`Egress control`,body:`none = isolated loopback. egress = outbound HTTPS allowed, filtered by the sandbox egress policy.`,icon:C,iconClass:`text-primary`},{field:`auth_mode`,purpose:`Invoke gate`,body:`none = public. platform_key = require Orva API key. signed = require HMAC.`,icon:T,iconClass:`text-primary`},{field:`rate_limit_per_min`,purpose:`Per-IP throttle`,body:`Optional cap for public or webhook-facing functions. Exceeding it returns 429.`,icon:ie,iconClass:`text-primary`}],ln=_(()=>`curl -X POST ${L.value}/api/v1/functions \\

...
}`,Is=`# 1. Generate an API key in the dashboard (Keys page) or via the API
...
orva system health      # smoke test`,js=`# Init a project in cwd (creates orva.yaml + handler stub)
orva init
...
orva deploy ./my-fn --name api --runtime python --entrypoint app.py`,Es=`# Invoke a function by name or fn_<id>:
...
print(r.json())`}]),ie=[{id:"python",name:"Python 3.14",entry:"handler.py",deps:"requirements.txt",icon:se},{id:"node",name:"Node.js 24",entry:"handler.js",deps:"package.json",icon:te}],le=[{field:"env_vars",purpose:"Plain config",body:"Plaintext config stored on the function record. Use for feature flags and non-secret settings.",icon:Ue,iconClass:"text-violet-300"},{field:"/secrets",purpose:"Encrypted",body:"AES-256-GCM at rest. Values decrypt only into the worker environment at spawn time.",icon:M,iconClass:"text-emerald-300"},{field:"network_mode",purpose:"Egress control",body:"none = isolated loopback. egress = outbound HTTPS allowed; firewall blocklist applies.",icon:K,iconClass:"text-sky-300"},{field:"auth_mode",purpose:"Invoke gate",body:"none = public. platform_key = require Orva API key. signed = require HMAC.",icon:Xe,iconClass:"text-violet-300"},{field:"rate_limit_per_min",purpose:"Per-IP throttle",body:"Optional cap for public or webhook-facing functions. Exceeding it returns 429.",icon:Be,iconClass:"text-amber-300"}],pe=v(()=>`curl -X POST ${r.value}/api/v1/functions \\
  -H 'X-Orva-API-Key: <YOUR_KEY>' \\
  -H 'Content-Type: application/json' \\
  -d '{"name":"hello","runtime":"python","memory_mb":128,"cpus":0.5}'`),ue=v(()=>`tar czf code.tar.gz handler.py requirements.txt
...
Auth: OAuth (auto-discovered)`}]),Ie=v(()=>{const c=JSON.stringify({url:r.value+"/mcp",headers:{Authorization:"Bearer "+m.value}});return typeof window.btoa=="function"?window.btoa(c):c}),je=v(()=>[{label:"Cursor (global)",lang:"json",note:"Paste into ~/.cursor/mcp.json, or .cursor/mcp.json in your project root for a per-workspace install.",code:`{
  "mcpServers": {
    "orva": {
      "url": "${r.value}/mcp",
      "headers": {

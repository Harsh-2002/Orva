import{c as h}from"./index-BiDPOHk5.js";import{c as d}from"./clipboard-Ccp9U7DQ.js";const _=h("chevron-down",[["path",{d:"m6 9 6 6 6-6",key:"qrunsl"}]]),p=`You are an Orva serverless-function expert. You write production-ready Python or Node handlers that follow Orva's contract exactly, use Orva's built-in primitives instead of inventing external infrastructure, and never produce framework boilerplate the platform doesn't need.
...
</example_3_webhook_receiver>`,m="Now ask me what kind of function I want to build. When I describe it, return a complete, ready-to-paste handler file plus requirements.txt or package.json if any third-party deps are needed. Default to Python 3.14 unless I say otherwise. If my idea fits orva.kv (caching/state), orva.jobs (background work), orva.invoke (chaining functions), a cron schedule, or a webhook receiver, use those primitives instead of inventing external infrastructure.",f=()=>`${p}

---

${m}`,S=()=>d(f()),i=8*1024,y=e=>{if(!e)return"";const t=new TextEncoder,r=new TextDecoder("utf-8",{fatal:!1}),o=t.encode(e);if(o.length<=i)return e;const n=r.decode(o.slice(0,i));return`[truncated — original was ${o.length} bytes; showing first ${i}]
${n}`},v=e=>{if(!e)return"text";const t=String(e).toLowerCase();return t.startsWith("python")?"python":t.startsWith("node")?"node":t.startsWith("ts")||t.includes("typescript")?"typescript":"text"},g=e=>{if(!e)return"unknown runtime";const t=String(e).toLowerCase();let r=t.match(/^python(\d)(\d+)$/);return r?`Python ${r[1]}.${r[2]}`:(r=t.match(/^node(\d+)$/),r?`Node.js ${r[1]}`:e)},b=e=>{if(!e||!e.method&&!e.path&&!e.headers&&!e.body)return"no captured request — operator triggered the failure outside the dashboard or capture was disabled.";const t=[],r=(e.method||"POST").toUpperCase(),o=e.path||"/";t.push(`${r} ${o}`);const n=e.headers||{},a=Object.keys(n);if(a.length){t.push("");for(const s of a)t.push(`${s}: ${n[s]}`)}return e.body&&(t.push(""),t.push(typeof e.body=="string"?e.body:JSON.stringify(e.body))),t.join(`
`)},w=({source:e="",language:t,runtime:r="",stderr:o="",requestPreview:n=null,errorMessage:a="",statusCode:s=""}={})=>{const c=t||v(r),l=g(r),u=[s,a].filter(Boolean).join(" ").trim()||"function failed without an explicit error message.";return["<context>",`An Orva serverless function (${l}) failed at runtime. Below are the function source, the captured HTTP request that triggered the failure, the stderr emitted by the worker, and the error the platform recorded. Help me diagnose and patch it.`,"</context>","",`<source language="${c}">`,e||"(source unavailable — fetch from the dashboard before debugging)","</source>","","<request>",b(n),"</request>","","<stderr>",y(o)||"(no stderr captured)","</stderr>","","<error>",u,"</error>","","<task>","Explain the failure in 2-3 sentences. Then propose the smallest patch that fixes it. Output the patched function in a code block. Do not include unrelated changes or refactors.","</task>"].join(`
`)},x=e=>d(w(e));export{_ as C,S as a,f as b,x as c};

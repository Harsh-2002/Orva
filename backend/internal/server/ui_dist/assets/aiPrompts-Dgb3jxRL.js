...
${p}`,k=()=>d(m()),i=8*1024,f=e=>{if(!e)return"";const t=new TextEncoder,r=new TextDecoder("utf-8",{fatal:!1}),o=t.encode(e);if(o.length<=i)return e;const n=r.decode(o.slice(0,i));return`[truncated — original was ${o.length} bytes; showing first ${i}]
...
`)},b=({source:e="",language:t,runtime:r="",stderr:o="",requestPreview:n=null,errorMessage:a="",statusCode:s=""}={})=>{const c=t||y(r),l=v(r),u=[s,a].filter(Boolean).join(" ").trim()||"function failed without an explicit error message.";return["<context>",`An Orva serverless function (${l}) failed at runtime. Below are the function source, the captured HTTP request that triggered the failure, the stderr emitted by the worker, and the error the platform recorded. Help me diagnose and patch it.`,"</context>","",`<source language="${c}">`,e||"(source unavailable — fetch from the dashboard before debugging)","</source>","","<request>",g(n),"</request>","","<stderr>",f(o)||"(no stderr captured)","</stderr>","","<error>",u,"</error>","","<task>","Explain the failure in 2-3 sentences. Then propose the smallest patch that fixes it. Output the patched function in a code block. Do not include unrelated changes or refactors.","</task>"].join(`

import{t as e}from"./clipboard-D_9N0yai.js";var t=`You are an Orva serverless-function expert. You write production-ready Python or Node handlers that follow Orva's contract exactly, use Orva's built-in primitives instead of inventing external infrastructure, and never produce framework boilerplate the platform doesn't need.
...
- When egress IS enabled, the operator can further restrict it with the sandbox egress policy + DNS settings (Egress controls page): blocked destinations are refused per sandbox, so a connection can come back as ECONNREFUSED. Assume best-effort; handle failures.
...
<channels>
Channels are how a deployed function gets exposed to a third-party agentic workflow as an MCP tool, without giving that workflow Orva-management authority.

The operator (NOT the handler) goes to /web/channels in the dashboard, picks a subset of deployed functions, names the bundle, and gets a static bearer token (orva_chn_<32 hex>). The agent receiving that token, when it presents it at /mcp, sees ONE invoke-only MCP tool per bundled function and nothing else — no list_functions, no deploy, no docs. Auth is either Authorization: Bearer or X-Orva-API-Key on /mcp; channel tokens are 401-rejected at /api/v1/*.

Tool name in the agent's catalog = the function's name with dashes converted to snake_case (stripe-charge → stripe_charge). The function's \`description\` field (visible in the Settings modal of the editor) becomes the tool description shown to the agent's planner. WHEN YOU GENERATE A FUNCTION the operator might later expose as a channel tool, write a clear one-line description for it — that text is what the downstream agent reads to decide when to call this tool. Use snake_case in name candidates if the function will primarily be used through a channel (so the human-readable form and the MCP tool name match exactly).

Channels do not change how you write the handler. The HTTP request the channel-tool sends is the standard event {method:"POST", path:"/", body:<args>} envelope you already write against.
</channels>
...
</example_3_webhook_receiver>`,n=`Now ask me what kind of function I want to build. When I describe it, return a complete, ready-to-paste handler file plus requirements.txt or package.json if any third-party deps are needed. Default to Python 3.14 unless I say otherwise. If my idea fits orva.kv (caching/state), orva.jobs (background work), orva.invoke (chaining functions), a cron schedule, or a webhook receiver, use those primitives instead of inventing external infrastructure.`,r=()=>`${t}\n\n---\n\n${n}`,i=()=>e(r()),a=8192,o=e=>{if(!e)return``;let t=new TextEncoder,n=new TextDecoder(`utf-8`,{fatal:!1}),r=t.encode(e);if(r.length<=a)return e;let i=n.decode(r.slice(0,a));return`[truncated — original was ${r.length} bytes; showing first ${a}]\n${i}`},s=e=>{if(!e)return`text`;let t=String(e).toLowerCase();return t.startsWith(`python`)?`python`:t.startsWith(`node`)?`node`:t.startsWith(`ts`)||t.includes(`typescript`)?`typescript`:`text`},c=e=>{if(!e)return`unknown runtime`;let t=String(e).toLowerCase();return t===`node`?`Node.js 24`:t===`python`?`Python 3.14`:e},l=e=>{if(!e||!e.method&&!e.path&&!e.headers&&!e.body)return`no captured request — operator triggered the failure outside the dashboard or capture was disabled.`;let t=[],n=(e.method||`POST`).toUpperCase(),r=e.path||`/`;t.push(`${n} ${r}`);let i=e.headers||{},a=Object.keys(i);if(a.length){t.push(``);for(let e of a)t.push(`${e}: ${i[e]}`)}return e.body&&(t.push(``),t.push(typeof e.body==`string`?e.body:JSON.stringify(e.body))),t.join(`
`)},u=({source:e=``,language:t,runtime:n=``,stderr:r=``,requestPreview:i=null,errorMessage:a=``,statusCode:u=``}={})=>{let d=t||s(n),f=c(n),p=[u,a].filter(Boolean).join(` `).trim()||`function failed without an explicit error message.`;return[`<context>`,`An Orva serverless function (${f}) failed at runtime. Below are the function source, the captured HTTP request that triggered the failure, the stderr emitted by the worker, and the error the platform recorded. Help me diagnose and patch it.`,`</context>`,``,`<source language="${d}">`,e||`(source unavailable — fetch from the dashboard before debugging)`,`</source>`,``,`<request>`,l(i),`</request>`,``,`<stderr>`,o(r)||`(no stderr captured)`,`</stderr>`,``,`<error>`,p,`</error>`,``,`<task>`,`Explain the failure in 2-3 sentences. Then propose the smallest patch that fixes it. Output the patched function in a code block. Do not include unrelated changes or refactors.`,`</task>`].join(`
`)},d=t=>e(u(t));export{d as n,i as r,r as t};
...
print(r.json())`}]),sn=[{id:`python`,name:`Python 3.14`,entry:`handler.py`,deps:`requirements.txt`,icon:$t},{id:`node`,name:`Node.js 24`,entry:`handler.js`,deps:`package.json`,icon:en}],cn=[{field:`env_vars`,purpose:`Plain config`,body:`Plaintext config stored on the function record. Use for feature flags and non-secret settings.`,icon:re,iconClass:`text-violet-300`},{field:`/secrets`,purpose:`Encrypted`,body:`AES-256-GCM at rest. Values decrypt only into the worker environment at spawn time.`,icon:w,iconClass:`text-emerald-300`},{field:`network_mode`,purpose:`Egress control`,body:`none = isolated loopback. egress = outbound HTTPS allowed, filtered by the sandbox egress policy.`,icon:C,iconClass:`text-sky-300`},{field:`auth_mode`,purpose:`Invoke gate`,body:`none = public. platform_key = require Orva API key. signed = require HMAC.`,icon:T,iconClass:`text-violet-300`},{field:`rate_limit_per_min`,purpose:`Per-IP throttle`,body:`Optional cap for public or webhook-facing functions. Exceeding it returns 429.`,icon:oe,iconClass:`text-amber-300`}],ln=v(()=>`curl -X POST ${L.value}/api/v1/functions \\
...
curl -sN -X POST ${L.value}/mcp \\
  ...
  -H 'Content-Type: application/json' \\
  -H 'Accept: application/json, text/event-stream' \\
  -d '{"jsonrpc":"2.0","id":3,"method":"tools/list","params":{}}'`}]),En=v(()=>[{label:`Claude Desktop`,lang:`json`,note:`Paste into ~/Library/Application Support/Claude/claude_desktop_config.json (macOS), %APPDATA%\\Claude\\claude_desktop_config.json (Windows), or ~/.config/Claude/claude_desktop_config.json (Linux). Restart Claude Desktop.`,code:`{
  "mcpServers": {
    "orva": {
      "url": "${L.value}/mcp",
      "headers": {

...
`,Xo=`# Verify a Stripe webhook and branch on event type.
...
`,Qo=`# Verify a GitHub webhook (HMAC-SHA256) and route on X-GitHub-Event.
...
`,Zo=`# Slack slash command handler. Slack POSTs application/x-www-form-urlencoded
...
`,es=`# Convert a CSV upload to JSON. Accepts plain text body OR a multipart
...
`,ts=`# Scheduled DB cleanup: delete rows older than RETENTION_DAYS.
...
`,os=`# RSS / Atom summarizer. Fetches a feed and returns the latest N items
...
`,ss=`# Resize an image to a thumbnail. Accepts {"image_b64": "...", "width": 200}
...
`,as=`# Streaming LLM-style token generator (v0.4 C1 showcase).
#
...
def handler(event):
    ...
    for word in fake_tokens:
        ...
        time.sleep(0.1)
    yield "\\n"
`,Ge=`exports.handler = async (event) => {
...
`,is=`// Discord interactions endpoint. Set DISCORD_PUBLIC_KEY in secrets.
...
`,ds=`// Verify a JWT (HS256) and return its decoded claims. Set JWT_SECRET
...
`,us=`// OAuth 2.0 callback — exchanges an authorization code for an access
...
`,cs=`// Convert markdown to safe HTML using marked + DOMPurify-like escaping.
// Body: {"markdown": "# Hello\\n..."}.
...
`,ps=`// REST proxy with auth header injection. Forwards every request to
...
`,fs=`// Daily email digest, fired by a Schedules entry like "0 9 * * *".
...
`,hs=`// URL shortener using Orva's built-in KV store (Phase 3).
...
`,gs=`// Minimal TypeScript handler. The Orva builder compiles this with tsc
...
`,xs=`"""v0.6 SDK showcase: trace.span + log.* + kv.incr + jobs.enqueue.
...
`,Rt=[{id:"py-http-hello",category:"Starter",label:"HTTP Hello",description:"Minimal POST /. Echoes a name from the JSON body.",code:He,deps:""},{id:"py-stripe-webhook",category:"Webhooks",label:"Stripe webhook",description:"Verify Stripe signature; branch on event.type.",code:Xo,deps:""},{id:"py-github-webhook",category:"Webhooks",label:"GitHub webhook",description:"HMAC-SHA256 verification; route on X-GitHub-Event.",code:Qo,deps:""},{id:"py-slack-bot",category:"Webhooks",label:"Slack slash command",description:"Parse application/x-www-form-urlencoded; respond inline.",code:Zo,deps:""},{id:"py-csv-to-json",category:"Utility",label:"CSV → JSON",description:"Parse a CSV upload and return a JSON array of rows.",code:es,deps:""},{id:"py-cron-cleanup",category:"Scheduled",label:"Scheduled DB cleanup",cron:!0,description:"Daily delete of rows older than RETENTION_DAYS.",code:ts,deps:""},{id:"py-rss-summarize",category:"Utility",label:"RSS summarizer",description:"Fetch an RSS / Atom feed and return latest N items.",code:os,deps:""},{id:"py-image-thumbnail",category:"Utility",label:"Image thumbnail",description:"Resize a base64 image with Pillow; returns PNG.",code:ss,deps:ns},{id:"py-guestbook",category:"Showcase",label:"Guestbook (full-stack showcase)",cron:!0,description:"HTML page + JSON API + KV + jobs + cron + secrets in one file. Best demo of what one Orva function can do.",code:rs,deps:""},{id:"py-stream-llm",category:"Showcase",label:"Streaming LLM tokens",description:"Generator that yields one word every 100ms. Demonstrates v0.4 chunked streaming end-to-end.",code:as,deps:""},{id:"py-traced-pipeline",category:"Showcase",label:"v0.6 SDK pipeline",description:"trace.span + log.info + kv.incr + idempotent jobs.enqueue. After invoking, open the new trace — child spans and structured logs render inline in the waterfall.",code:xs,deps:""}],_s=`// v0.6 SDK showcase: kv.incr + kv.cas + log.info.

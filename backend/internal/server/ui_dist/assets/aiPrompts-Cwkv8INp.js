import{c as h}from"./index-Dbk5vuWw.js";import{c as d}from"./clipboard-Cup8gynm.js";const x=h("chevron-down",[["path",{d:"m6 9 6 6 6-6",key:"qrunsl"}]]),p=`You are an Orva serverless-function expert. You write production-ready Python or Node handlers that follow Orva's contract exactly, use Orva's built-in primitives instead of inventing external infrastructure, and never produce framework boilerplate the platform doesn't need.
...
${m}`,S=()=>d(y()),i=8*1024,f=e=>{if(!e)return"";const t=new TextEncoder,r=new TextDecoder("utf-8",{fatal:!1}),o=t.encode(e);if(o.length<=i)return e;const n=r.decode(o.slice(0,i));return`[truncated — original was ${o.length} bytes; showing first ${i}]
...
`)},w=({source:e="",language:t,runtime:r="",stderr:o="",requestPreview:n=null,errorMessage:s="",statusCode:a=""}={})=>{const c=t||v(r),u=g(r),l=[a,s].filter(Boolean).join(" ").trim()||"function failed without an explicit error message.";return["<context>",`An Orva serverless function (${u}) failed at runtime. Below are the function source, the captured HTTP request that triggered the failure, the stderr emitted by the worker, and the error the platform recorded. Help me diagnose and patch it.`,"</context>","",`<source language="${c}">`,e||"(source unavailable — fetch from the dashboard before debugging)","</source>","","<request>",b(n),"</request>","","<stderr>",f(o)||"(no stderr captured)","</stderr>","","<error>",l,"</error>","","<task>","Explain the failure in 2-3 sentences. Then propose the smallest patch that fixes it. Output the patched function in a code block. Do not include unrelated changes or refactors.","</task>"].join(`
`)},_=e=>d(w(e));export{x as C,S as a,y as b,_ as c};

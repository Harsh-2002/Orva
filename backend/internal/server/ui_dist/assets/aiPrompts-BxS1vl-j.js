import{c as h}from"./index-Dw0D41kA.js";import{c as d}from"./clipboard-CzK6iwVa.js";const x=h("chevron-down",[["path",{d:"m6 9 6 6 6-6",key:"qrunsl"}]]),p=`You are an Orva serverless-function expert. You write production-ready Python or Node handlers that follow Orva's contract exactly, use Orva's built-in primitives instead of inventing external infrastructure, and never produce framework boilerplate the platform doesn't need.
...
</example_3_webhook_receiver>`,m="Now ask me what kind of function I want to build. When I describe it, return a complete, ready-to-paste handler file plus requirements.txt or package.json if any third-party deps are needed. Default to Python 3.14 unless I say otherwise. If my idea fits orva.kv (caching/state), orva.jobs (background work), orva.invoke (chaining functions), a cron schedule, or a webhook receiver, use those primitives instead of inventing external infrastructure.",y=()=>`${p}

---

${m}`,_=()=>d(y()),i=8*1024,f=e=>{if(!e)return"";const t=new TextEncoder,o=new TextDecoder("utf-8",{fatal:!1}),r=t.encode(e);if(r.length<=i)return e;const n=o.decode(r.slice(0,i));return`[truncated — original was ${r.length} bytes; showing first ${i}]
${n}`},v=e=>{if(!e)return"text";const t=String(e).toLowerCase();return t.startsWith("python")?"python":t.startsWith("node")?"node":t.startsWith("ts")||t.includes("typescript")?"typescript":"text"},g=e=>{if(!e)return"unknown runtime";const t=String(e).toLowerCase();let o=t.match(/^python(\d)(\d+)$/);return o?`Python ${o[1]}.${o[2]}`:(o=t.match(/^node(\d+)$/),o?`Node.js ${o[1]}`:e)},b=e=>{if(!e||!e.method&&!e.path&&!e.headers&&!e.body)return"no captured request — operator triggered the failure outside the dashboard or capture was disabled.";const t=[],o=(e.method||"POST").toUpperCase(),r=e.path||"/";t.push(`${o} ${r}`);const n=e.headers||{},s=Object.keys(n);if(s.length){t.push("");for(const a of s)t.push(`${a}: ${n[a]}`)}return e.body&&(t.push(""),t.push(typeof e.body=="string"?e.body:JSON.stringify(e.body))),t.join(`
`)},w=({source:e="",language:t,runtime:o="",stderr:r="",requestPreview:n=null,errorMessage:s="",statusCode:a=""}={})=>{const c=t||v(o),l=g(o),u=[a,s].filter(Boolean).join(" ").trim()||"function failed without an explicit error message.";return["<context>",`An Orva serverless function (${l}) failed at runtime. Below are the function source, the captured HTTP request that triggered the failure, the stderr emitted by the worker, and the error the platform recorded. Help me diagnose and patch it.`,"</context>","",`<source language="${c}">`,e||"(source unavailable — fetch from the dashboard before debugging)","</source>","","<request>",b(n),"</request>","","<stderr>",f(r)||"(no stderr captured)","</stderr>","","<error>",u,"</error>","","<task>","Explain the failure in 2-3 sentences. Then propose the smallest patch that fixes it. Output the patched function in a code block. Do not include unrelated changes or refactors.","</task>"].join(`
`)},S=e=>d(w(e));export{x as C,_ as a,y as b,S as c};

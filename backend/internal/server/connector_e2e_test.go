...
// TestConnectorE2E_TokenRejectedAtRESTAPI — connector tokens have no
// business hitting /api/v1/*. The auth middleware must explicitly
// reject `orva_aco_*` Bearer tokens with 401.
func TestConnectorE2E_TokenRejectedAtRESTAPI(t *testing.T) {
	tc := newTestServer(t)

	for _, path := range []string{"/api/v1/functions", "/api/v1/system/storage", "/api/v1/connectors"} {
		req := httptest.NewRequest("GET", path, nil)
		req.Header.Set("Authorization", "Bearer orva_aco_0123456789abcdef0123456789abcdef")
		w := httptest.NewRecorder()
		tc.srv.router.ServeHTTP(w, req)
		if w.Code != http.StatusUnauthorized {
			t.Errorf("%s with connector bearer: status=%d (want 401)", path, w.Code)
		}
		// And the message should be specific so a confused operator
		// gets a clear hint.
		if !strings.Contains(w.Body.String(), "connector tokens are MCP-only") {
			t.Errorf("%s: error message doesn't mention connector restriction; body=%s",
				path, w.Body.String())
		}
	}
}

// TestConnectorE2E_FullFlow — create function trio, bundle two of them
// into a connector, confirm the connector token authenticates at /mcp,
// confirm the third (non-bundled) function is NOT exposed.
func TestConnectorE2E_FullFlow(t *testing.T) {
	tc := newTestServer(t)

	// Seed three functions directly via the DAL (skipping the deploy
	// pipeline keeps the test focused on the connector behavior).
	...
	// Create a connector containing email + summarize (NOT the
	// dangerous one).
	createBody, _ := json.Marshal(map[string]any{
		"name":         "support-bot",
		"function_ids": []string{emailID, summarizeID},
	})
	req := httptest.NewRequest("POST", "/api/v1/connectors", bytes.NewReader(createBody))
	...
	if !strings.HasPrefix(connToken, "orva_aco_") {
		t.Fatalf("token has wrong prefix: %q", connToken)
	}

	// Initialize MCP session with the connector token.
	...
	if w.Code != http.StatusOK {
		t.Fatalf("mcp initialize w/ connector token: status=%d body=%s", w.Code, w.Body.String())
	}
	...
	for _, want := range []string{`"email_sender"`, `"summarize_text"`} {
		if !strings.Contains(body, want) {
			t.Errorf("tools/list missing %q\nbody excerpt: %s", want, truncateConnectorBody(body))
		}
	}
	...
	for _, forbidden := range []string{
		...
		if strings.Contains(body, forbidden) {
			t.Errorf("tools/list LEAKED %q to connector token\nbody excerpt: %s",
				forbidden, truncateConnectorBody(body))
		}
	}
}

// TestConnectorE2E_ToolNameCollisionRejected — two function names
// that snake_case to the same MCP tool name should be rejected at
// create time with 400.
func TestConnectorE2E_ToolNameCollisionRejected(t *testing.T) {
	tc := newTestServer(t)
...
// TestConnectorE2E_RotateInvalidatesOldToken — rotate the connector,
// confirm old token 401s and new token works.
func TestConnectorE2E_RotateInvalidatesOldToken(t *testing.T) {
	tc := newTestServer(t)
	...
	// Rotate.
	req = httptest.NewRequest("POST", "/api/v1/connectors/"+connID+"/rotate", nil)
	tc.setAuth(req)
	w = httptest.NewRecorder()
	tc.srv.router.ServeHTTP(w, req)
	if w.Code != http.StatusOK {
...
// TestConnectorE2E_PrincipalTypeWiresUp — Principal type compiles and
// the auth Kind constants are in sync (regression guard against
// future renames that would silently break connector dispatch).
func TestConnectorE2E_PrincipalTypeWiresUp(t *testing.T) {
	if auth.KindConnector != "connector" {
		t.Errorf("KindConnector = %q, want connector", auth.KindConnector)
	}
	if auth.KindAPIKey != "api_key" {
		t.Errorf("KindAPIKey = %q, want api_key", auth.KindAPIKey)
	}
	p := &auth.Principal{Kind: auth.KindConnector, Connector: &auth.ConnectorRef{}}
	if p.Connector == nil {
		t.Error("ConnectorRef must be settable on Principal")
	}
}

// ── helpers ──────────────────────────────────────────────────────────

func truncateConnectorBody(s string) string {
	if len(s) <= 800 {
		return s
	}
	return s[:800] + "..."

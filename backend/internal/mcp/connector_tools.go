...
// registerConnectorTools registers exactly one MCP tool per function
// in the connector's bundle, all routed through the existing
// invokeFunction primitive. NO Orva-management tools are registered —
// the connector token has no Orva-management authority.
//
// Tool name: function name with dashes converted to underscores
// (`stripe-charge` → `stripe_charge`). Collision is prevented at
// connector-create/update time by the DAL's
// CheckConnectorToolNameCollision; we trust that here.
//
// Tool description: per-connector override (junction-row description)
...
func registerConnectorTools(s *mcpsdk.Server, deps Deps, ref *authpkg.ConnectorRef) {
	if ref == nil || len(ref.FunctionIDs) == 0 {
		return
	}

	// Pull per-connector descriptions in one shot so we don't query
	// once per function during registration.
	descByFnID := make(map[string]string, len(ref.FunctionIDs))
	if rows, err := deps.DB.ListConnectorFunctionRecords(ref.ID); err == nil {
		for _, cf := range rows {
			if cf.Description != "" {
	...
	for _, fnID := range ref.FunctionIDs {
		...
		if err != nil {
			...
			// cleaned up yet (CASCADE drops it on the next /connectors
		...
		mcpsdk.AddTool(s,
			...
			func(ctx context.Context, _ *mcpsdk.CallToolRequest, in map[string]any) (*mcpsdk.CallToolResult, InvokeFunctionOutput, error) {
				...
				// connector boundary already gated the surface
...
// buildConnectorInstructions generates the per-connector serverInstructions
// block. If the connector row carries a non-empty `instructions` value
...
func buildConnectorInstructions(db *database.Database, ref *authpkg.ConnectorRef) string {
	if ref == nil {
		return ""
	}
	if ref.Instructions != "" {
	...
	fmt.Fprintf(&b, "This connector exposes the **%s** toolset on Orva.", ref.Name)
	if ref.Description != "" {
	...
	descByFnID := make(map[string]string, len(ref.FunctionIDs))
	if rows, err := db.ListConnectorFunctionRecords(ref.ID); err == nil {
		for _, cf := range rows {
			if cf.Description != "" {
				descByFnID[cf.FunctionID] = cf.Description
			}
	...
	b.WriteString("\nEach tool invokes a deployed serverless function. " +
		...
		"No platform-management tools are available — this connector is invoke-only.")
	return b.String()
}

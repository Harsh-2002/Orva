...
| `mcp` | MCP server (go-sdk); 73 operator-management tools OR channel-mode (one tool per bundled function, invoke-only). Auth accepts API keys, OAuth 2.1 access tokens, OR channel tokens. **Transport is stateless** (`StreamableHTTPOptions{Stateless: true}`) — the SDK serves protocol `2026-07-28` only in that mode, and older clients still negotiate down. No `initialize` handshake required, no `Mcp-Session-Id`, `GET`/`DELETE /mcp` → 405. A `*Server` is therefore built **per request**, so a process-wide `ServerOptions.SchemaCache` keeps ~73 `AddTool` reflections off the hot path, and the auth gate stashes the resolved `*Principal` in the request context for `getServer` instead of resolving twice. `cacheScopeMiddleware` rewrites the SDK's hardcoded `cacheScope: "public"` to `"private"` — the catalog is permission- and channel-scoped, so a shared cache entry would leak one principal's tool surface to another. |

<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between gap-4">
      <div>
        ...
        <p class="text-xs text-foreground-muted mt-1 max-w-prose">
          ...
          agentic workflow. Each connector has its own bearer token;
    ...
    <!-- Risk banner. The connector boundary covers the MCP surface,
    ...
    <div class="rounded-md border border-amber-700/30 bg-amber-950/15 px-4 py-3 text-xs text-amber-200/90 leading-relaxed">
      ...
      Connector tokens grant invoke-only MCP access to the bundled
      functions. Bundled functions remain as powerful as you've
      configured them — including any Orva REST SDK calls they make
      from inside their sandbox. The connector boundary protects the
    ...
    >
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div>
          ...
          <select
            v-model="newConnector.expiresInDays"
            class="w-full bg-surface-hover border border-border rounded-md px-3 py-2 text-sm text-foreground focus:outline-none focus:border-white"
          >
            <option :value="0">
              Never
      ...
      <div>
        ...
        <input
          v-model="newConnector.description"
      ...
      <div>
        <label class="text-xs font-medium text-foreground-muted uppercase tracking-wide block mb-1.5">
          Functions <span class="text-white">({{ newConnector.functionIds.length }} selected)</span>
        </label>
        <Button
          variant="secondary"
        ...
        >
          <Boxes class="w-4 h-4" />
          {{ newConnector.functionIds.length === 0 ? 'Pick functions' : 'Edit selection' }}
        </Button>
      </div>
      <div
        v-if="createError"
      ...
      <table class="w-full text-sm text-left">
        ...
        <tbody class="divide-y divide-border">
          ...
          <tr v-if="connectors.length === 0">
            ...
            >
              No connectors yet. Click <span class="text-white">New Connector</span> to bundle functions for an agent.
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <FunctionPickerModal
      v-if="pickerOpen"
      :selected="newConnector.functionIds"
...
import FunctionPickerModal from '@/components/connectors/FunctionPickerModal.vue'
import {
  listConnectors,
  createConnector,
  rotateConnector,
...
const canSubmit = computed(
  () => newConnector.value.name.trim() && newConnector.value.functionIds.length > 0,
)
...
const openCreate = () => {
  newConnector.value = { name: '', description: '', expiresInDays: 0, functionIds: [] }
  createError.value = ''
  creating.value = true
}
const cancelCreate = () => {
...
const onPickerApply = (ids) => {
  newConnector.value.functionIds = ids
...
const submitCreate = async () => {
  ...
  try {
    const body = {
      name: newConnector.value.name.trim(),
      description: newConnector.value.description.trim(),
      function_ids: newConnector.value.functionIds,
    }
    if (newConnector.value.expiresInDays > 0) {
      body.expires_in_days = newConnector.value.expiresInDays
    }
    const res = await createConnector(body)
    createdToken.value = res.data.token
    creating.value = false
    await load()
  } catch (err) {
    createError.value = err?.response?.data?.error?.message || 'Failed to create connector.'
  } finally {
...
const rotate = async (c) => {
  ...
  if (!ok) return
  const res = await rotateConnector(c.id)
  createdToken.value = res.data.token
  await load()
}
...
const remove = async (c) => {
  const ok = await confirmStore.ask({
    ...
    message:
      ...
      'are not affected. Re-create the connector if you need it again.',
    confirmLabel: 'Delete',
    danger: true,
  })
  if (!ok) return

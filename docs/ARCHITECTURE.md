...
Migrations are additive only — `CREATE TABLE IF NOT EXISTS` + idempotent ALTER — with two carve-outs. First, `migrate_to_uuidv7.go` rewrites primary keys, derives its FK children from `PRAGMA foreign_key_list`, rewrites seven undeclared soft references, and renames `<dataDir>/functions/<id>/` to match, driven by a map committed in the same transaction columns run on every boot. Second, `reclaimOrphanedExecutionRows` deletes `executions` rows whose function no longer exists and `execution_id`-keyed child rows whose execution no longer exists — an irreversible bulk `DELETE` of rows nothing can reach, batched at 5,000 rows and capped at 1,000,000 per table per boot, gated one-shot behind the `orphan_reclaim_done` row in `system_config`. AI message/tool-call rows cascade-delete with their `ai_conversations` parent; editing or deleting a chat message truncates the conversation tail by `seq` (see `DeleteMessagesFromSeq`).

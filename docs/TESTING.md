...
The run writes a local, gitignored `test/e2e/CHECKLIST.md`; CI publishes its
copy in the job summary. For an already-provisioned scratch instance, use `--url` instead:
...
That is also how CI runs it. The checklist describes one run, not a repository
baseline or a coverage inventory.
...
Modules are discovered from `test/e2e/tests/test_*.py`; inspect that directory
for the current inventory. Runtime check counts vary where loops are involved.
Stdlib only.
...
> **A filtered run overwrites the local checklist with partial results.**
> `main()` calls `write_checklist(results, …)` with only the filtered modules.
> To inspect one module without replacing the last full-run report, use §3.2.4.
...
A local, gitignored file, regenerated by `write_checklist()` on every run. It
...
Do not read `CHECKLIST.md` as the coverage inventory: a filtered run replaces it
with a partial report. Discover modules from `test/e2e/tests/` instead.
...
7. `: > test/e2e/CHECKLIST.md` — clear any previous local report before the job,
   then `run.py --url http://127.0.0.1:8443 --api-key "$ADMIN_KEY"`
...
## 9. Historical audit notes (verify current status)

Historical audit notes: some entries may have been fixed since they were
recorded. Verify each against current source before treating it as an open
defect; fix rather than route around a defect that still exists.
...
| `test/kata-bench/*`, `test/install/kata-flow.sh` | Default to `ghcr.io/harsh-2002/orva:latest` (F5 fixed). `extended-functional.sh`'s header advertised five legs against a body of three; it now names the three it runs (baseline, egress, secrets) and says kv/cron/f2f were never implemented. |
...
| Don't | Because |
|---|---|
| `python3 run.py --filter …` | replaces the local `CHECKLIST.md` with partial results |

...
- **One published release at a time.** The previous releases are deleted after a new
  ...
  the reverse order 404s every install and upgrade in the gap. **Wait for the
  `artifacts` suite to finish before pruning**, not merely for the release to
  publish: its CLI-upgrade leg resolves the *previous* release and downloads that
  release's assets, so deleting mid-run 404s it. That is not hypothetical — it is
  how v2026.08.26's artifacts run went red, from a prune fifteen seconds after the
  job had resolved its target. **The tag is pruned
